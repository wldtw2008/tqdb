...
void vCheckSystex(int argc, char *argv[]) {
	...
	{
		printf("Systex Error\n");
		printf("Systex: %s IP Port dbname dbgflag Symbol DateTimeBeg DateTimeEnd EPID\n", argv[0]);
		printf("    Ex: %s 127.0.0.1 9042 tqdb1.tick 1 WTX 2015-03-05 2015-03-06 0\n", argv[0]);
		exit(0);
	}
}
					...
					if (type ==1)
					{
						...
						if (iEPIDFilter==-1 || iEPIDFilter==objTickData.iEPID)
						{
							...
							if (iDBGFlag)
							{
								...
								printf("%d,%06d.%03d,%s,%d,%d\n",iDate, iTime, iMSec, szDb2Str(szPrc, &objTickData.dbPrc), objTickData.iVol,objTickData.iEPID);
							}
							else

								printf("%d,%06d.%03d,%s,%d\n", iDate, iTime, iMSec, szDb2Str(szPrc, &objTickData.dbPrc), objTickData.iVol);	
						}
						iDataCnt++;
					}
				}
