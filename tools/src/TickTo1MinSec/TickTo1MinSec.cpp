...
int getSecEpochFromDateTime(int iDateYYYYMMDD, int iTimeHHMMSS)
{
        struct tm t = {0};  // Initalize to all 0's
        t.tm_year = iDateYYYYMMDD/10000-1900;  // This is year-1900, so 112 = 2012
        t.tm_mon = ((iDateYYYYMMDD/100)%100)-1;
...
void vGetDateTimeFromSecEpoch(int iSecEpoch, int *piDateYYYYMMDD, int *piTimeHHMMSS)
{
        time_t timeSinceEpoch = iSecEpoch;
        struct tm* tmTime = localtime(&timeSinceEpoch);
        *piDateYYYYMMDD = (tmTime->tm_year+1900)*10000+(tmTime->tm_mon+1)*100+tmTime->tm_mday;
        *piTimeHHMMSS = tmTime->tm_hour*10000+tmTime->tm_min*100+tmTime->tm_sec;
}
...
iInterval=1 -->Sec
*/
int iGoParse1MinFromStdin(char* pszSymbol, int iDebug, int iFmtId, int iInterval)
{
	char* pszDate;
	char* pszTime;
	char* pszTickPrc;
	...
	while(fgets(szLine, sizeof(szLine), stdin))
	{
		...
		int iEncodeTradeDateTime = 0; //getMinEpochFromDateTime(iDataTradeDate, iTradeTime)+1;//+1min 
                switch (iInterval)
		{
			case 1:
				iEncodeTradeDateTime = getSecEpochFromDateTime(iDataTradeDate, iTradeTime)+1;//+1Sec
				break;
			case 0:
			default:
				iEncodeTradeDateTime = getMinEpochFromDateTime(iDataTradeDate, iTradeTime)+1;//+1min
				break;
		}
	...
	if(!objTimeTradeData.empty())
	{
		for(mapItTimeTradeData = objTimeTradeData.begin();mapItTimeTradeData!=objTimeTradeData.end();mapItTimeTradeData++)
		{
			...
			{
			case 1:
				vGetDateTimeFromSecEpoch(*piEncodeTradeDateTime, &iTradingDate, &iTradingTime);
				break;
			case 0:
			default:
			...
			if (iFmtId == 1)
				fprintf(stdout, "%04d-%02d-%02d %02d:%02d:%02d,%s,%s,%s,%s,%d\n", iTradingDate/10000, (iTradingDate/100)%100, iTradingDate%100,
					iTradingTime/10000, (iTradingTime/100)%100, iTradingTime%100,
...
#endif
{
	...
	else
	{
		...
		iGoParse1MinFromStdin((char*)"", iDebug, iFmtId, iInterval);
	}
